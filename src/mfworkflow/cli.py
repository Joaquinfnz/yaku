...
from mfworkflow import __version__
...
def find_templates_dir() -> Path:
    ...
    candidates = []
    # 1) relativo al paquete instalado en modo editable: <repo>/src/mfworkflow
...
def _load(project: str):
    ...
    from mfworkflow.logging_setup import setup_logging
...
def _validate(cfg, builder, logger) -> bool:
    ...
    from mfworkflow.builder import validate_geometry_and_units
    from mfworkflow.insumos import revisar_insumos
...
def cmd_report(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.report import generar_informe
...
def cmd_entregables(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.report import armar_entregables
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.calibration import evaluate_fit, run_pest, setup_pest
...
def cmd_transport(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.transport import gwt
...
def cmd_salina(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.transport import seawater
...
def cmd_pathlines(args: argparse.Namespace) -> int:
    ...
    from mfworkflow.pathlines import darcy_gradient
...
def cmd_gis(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.gis import preprocess
...
def cmd_mesh(args: argparse.Namespace) -> int:
    ...
    from mfworkflow.mesh import build_voronoi
...
def _correr_disv(cfg, gridprops, logger) -> None:
    ...
    from mfworkflow.gis.preprocess import find_vector
    from mfworkflow.mesh.voronoi import build_disv_model, run_disv_flow, zonas_geologicas
...
def cmd_recarga(args: argparse.Namespace) -> int:
    cfg, logger = _load(args.project)
    from mfworkflow.prep.recarga import calcular_recarga
...
def cmd_datos(args: argparse.Namespace) -> int:
    ...
    from mfworkflow.insumos import IMPORTANTE, OBLIGATORIO, formatear, revisar_insumos
    from mfworkflow.prep.skeletons import PLANTILLAS, crear_plantilla
    from mfworkflow.tui import UI
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    cfg, _ = _load(args.project)
    from mfworkflow.insumos import formatear, revisar_insumos
...
def cmd_onboard(args: argparse.Namespace) -> int:
    ...
    from mfworkflow.tui import run_home
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"mfworkflow {__version__}")
