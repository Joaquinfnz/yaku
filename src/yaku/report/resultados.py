...
from yaku.report.figuras import (  # noqa: E402, F401  (compat: se re-exportan)
    _dibujar_mapa,
    _niveles_isopiezas,
    _norte_y_escala,
    _paso_redondo,
...
class Resultados:
    ...
    balance_por_zonas: dict | None = None  # {df, csv, figura, n_zonas} balance estilo ZoneBudget
...
def recolectar_resultados(cfg, figuras_dir: Path | None = None) -> Resultados:
    ...
    # --- Balance por zonas (estilo ZoneBudget, desde el .cbc) ---
    if r.head is not None and np.asarray(r.head).ndim == 3:
        try:
            from yaku.report.zonas import balance_zonas_proyecto
            r.balance_por_zonas = balance_zonas_proyecto(cfg, figuras_dir)
        except Exception as exc:  # noqa: BLE001
            logger.warning("No se pudo calcular el balance por zonas: %s", exc)
    ...
    # --- Malla Voronoi no estructurada (DISV), si se corrió 'yaku mesh --run' ---
    malla = res_dir / "malla"
    ...
    # --- Sensibilidad (si se corrió 'yaku sensibilidad') ---
